...
public class EmailService {
	...
	public void sendMessage(String to, EmailType et) {
		...
		MimeMessage message = createMimeMessage(session, from, to, code);

		try {
			Transport transport = session.getTransport();
			transport.connect(from, fromSecret);
			transport.sendMessage(message, message.getAllRecipients());
			transport.close();
			
			CompletableFuture.runAsync(() -> {
				RegisterMessage rm = RegisterMessage.builder().requestId(
						RequestContextHolder.getRequestAttributes().getSessionId())
					.code(code).email(to).createdDate(LocalDateTime.now()).build();
	...
	public MimeMessage createMimeMessage(Session session, String from, String to, String code) {
		...
		try {
			...
			m.setContent(getContent(to, "注册账号", code), "text/html;charset=UTF-8");
			m.saveChanges();
	...
	public String getContent(String to, String ops, String qrcode) {
